...
* La sentencia `if` evalúa una condición (una e`xpresión` que resulta en `Verdadero o Falso`). Si la condición es verdadera, se ejecuta el bloque de código dentro de la instrucción `if`. Si la condición es falsa, se omite el bloque de código.
