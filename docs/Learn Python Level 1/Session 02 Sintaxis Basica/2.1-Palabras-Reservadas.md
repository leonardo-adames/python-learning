...
<div align="center">  
<kbd>
  <kbd>PALABRAS RESERVADAS [KEYWORDS]</kbd>
<br>
<br>
    
|`False`| `break` | `for` | `not` | `None` | `class` |
|-----|-------|-----|-----|------|-------|
| `from` | `or`  | `True` | `continue` | `global` | `pass` |
| `__peg_parser__` | `def` | `if` | `raise` | `and` | `del` | `import` |
| `return` | `as` | `elif` | `in` | `try` | `assert` | `else` | `is` | `while` |
...
**Lista Principal de Palabras Reservadas (Python 3):**

* **Booleanos/Nulos:** `False`, `None`, `True`.
* **Lógicos:** `and`, `or`, `not`.
* **Estructuras de Control:** `if`, `elif`, `else`, `for`, `while`, `break`, `continue`, `return`, `match`, `case`.
* **Definición de Elementos:** `def`, `class`, `lambda`.
* **Manejo de Excepciones:** `try`, `except`, `finally`, `raise`, `assert`.
* **Importación/Alias:** `import`, `from`, `as`.
* **Gestión de Variables:** `del`, `global`, `nonlocal`.
* **Asincronía:** `async`, `await`.
* **Otros:** `with`, `pass`, `yield`. 
...
| `match`          | Inicia una estructura de coincidencia de patrones.           | `match status_code:`             |
| `case`           | Define un patrón específico dentro de un `match`             | `case 404:print("No encontrado")`|
