...
Anteriormenrte Vímos que Los `Parámetros` en Python son `variables` que se definen en la cabecera de una función y que reciben valores cuando se llama a la función estos valores se les denomina `Argumentos`. Los parámetros permiten que las funciones sean más `flexibles` y `reutilizables`, ya que pueden operar con diferentes datos en cada invocación. La `sintaxis` básica para definir una función con parámetros en Python es la siguiente:
...
También vimos que En Python, existen diferentes tipos de parámetros que permiten una mayor flexibilidad a la hora de definir funciones. Los tipos principales son:
...
**LOS PARAMETROS `ARBITRARIOS` Y `POR VALOR  REFERENCIA`**
---
<br>

## Tipos de parametros en funciones definidas Avanzadas

Aquí veremos los 2 tipos de parametros de las funciones definidas que son los parametros `Arbitriarios` & los parametros por `Por valor & Por referencia`
<br>
