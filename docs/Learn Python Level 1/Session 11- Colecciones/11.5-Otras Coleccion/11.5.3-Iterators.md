...
Un `iterador` es un objeto que contiene un número contable de valores. Un `iterador` es un objeto sobre el que se puede iterar, lo que significa que se pueden recorrer todos los valores. Técnicamente, en Python, un `iterador` es un objeto que implementa el protocolo de iterador, que consta de los métodos `iter()` y `next()`.
